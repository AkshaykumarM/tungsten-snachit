...
#import "OADataFetcherTW.h"
#import "OAServiceTicketTW.h"
#import "OAMutableURLRequestTW.h"
#import "OACallTW.h"

@interface OACallTW (Private)

- (void)callFinished:(OAServiceTicketTW *)ticket withData:(NSData *)data;
- (void)callFailed:(OAServiceTicketTW *)ticket withError:(NSError *)error;

@end

@implementation OACallTW
...
- (void)callFailed:(OAServiceTicketTW *)aTicket withError:(NSError *)error {
	NSLog(@"error body: %@", aTicket.body);
	self.ticket = aTicket;
	[aTicket release];
	OAProblemTW *problem = [OAProblemTW problemWithResponseBody:ticket.body];
	if (problem) {
...
- (void)callFinished:(OAServiceTicketTW *)aTicket withData:(NSData *)data {
	self.ticket = aTicket;
	[aTicket release];
...

{
	...
	request = [[OAMutableURLRequestTW alloc] initWithURL:url
											  consumer:consumer
												token:token
												 realm:realm
...
//		}
//	}
	fetcher = [[OADataFetcherTW alloc] init];
